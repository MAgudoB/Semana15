<?php
    ...
    $query = "SELECT RoomId FROM `Rooms` WHERE RoomId = '".$_GET['RoomId']."'";
	...
	if ($result->num_rows > 0) {
		...
		$query2 = "SELECT COUNT(*) as nPlayers FROM `Rooms` WHERE roomId = '".$_GET['RoomId']."'";
		$result2 = $mysqli->query($query2);
		$row = $result->fetch_assoc();
		$playersInRoom = $row['nPlayers'];
		//Si estan los dos, comprobar si eres uno de los dos
		if ($playersInRoom == 2) {
			$queryFullRoom = "SELECT RoomId FROM `Rooms` WHERE RoomId = '".$_GET['RoomId']."' AND UserId = '".$_GET['UserId']."'";
			$resultFullRoom = $mysqli->query($queryFullRoom);
			//Si eres puedes jugar
			if ($result->num_rows > 0) {
				$queryUpdateActive = "UPDATE `Rooms` SET Active=1 WHERE RoomId = '".$_GET['RoomId']."' AND UserId = '".$_GET['UserId']."'";
		...
		//Si no está llena se te añade.
			...
			$queryInsert = "INSERT INTO `Rooms` (RoomId, UserId,Active) VALUES ('".$_GET['RoomId']."','".$_GET['UserID']."',1)";
